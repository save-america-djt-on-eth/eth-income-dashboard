...
async function updateCache() {
  ...
  try {
    ...
    const generateData = async (timeFrame) => {
      ...
      return {
        labels: generateTimeLabels(days, interval), // Generate time labels
        ethAddedNonDJT, // Non-DJT ETH added during the timeframe
        ethGeneratedByDJT: cumulativeEthGenerated, // DJT generated ETH (cumulative)
        currentEthTotal: currentEthBalance,
        newEthHoldings: ethAddedNonDJT[ethAddedNonDJT.length - 1], // Total ETH added (excluding DJT)
        newEthGeneratedDJT: cumulativeEthGenerated[cumulativeEthGenerated.length - 1] // Total DJT ETH
      };
    };
...
app.get('/api/data', (req, res) => {
  ...
  if (cache[timeFrame]) {
    if (simulate && simulate === 'false') {
      ...
      simulatedData.ethGeneratedByDJT = simulatedData.ethGeneratedByDJT.map(value => value * 1.1); // Example simulation
