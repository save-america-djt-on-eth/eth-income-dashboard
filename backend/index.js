...
async function updateCache() {
  ...
  try {
    ...
    const generateData = async (timeFrame) => {
      ...
      const contractTransactions = await fetchInternalTransactionsEtherscan(contractAddress);

      // Calculate cumulative ETH generated by DJT
      const cumulativeEthGeneratedByDJT = calculateCumulativeEthGenerated(contractTransactions, interval);

      // Calculate ETH generated by DJT during the timeframe
      cumulativeEthGeneratedByDJT.forEach((value, index) => {
        if (index > 0) {
          ethGeneratedByDJT.push(value - cumulativeEthGeneratedByDJT[index - 1]);
        } else {
      ...
      const cumulativeEthGeneratedByDJTFinal = ethGeneratedByDJT.reduce((acc, value, index) => {
        if (index === 0) {
      ...
      return {
        ...
        ethGeneratedByDJT: cumulativeEthGeneratedByDJTFinal,
        currentEthTotal: currentEthBalance,
        newEthHoldings: cumulativeEthAddedDuringTimeFrame,
        newEthGeneratedDJT: cumulativeEthGeneratedByDJTFinal
      };
    };
