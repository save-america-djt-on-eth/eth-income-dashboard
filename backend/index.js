  ...
  try {
    ...
    const generateData = async (timeFrame) => {
      ...
      for (let i = 0; i <= interval; i++) {
        ...
        trumpEtherIncomeDuringTimeFrame.unshift(ethBalance);
      }
      ...
      cumulativeEthGeneratedByDJT.forEach((value, index) => {
        if (index > 0) {
          etherIncomeFromContract.push(value - cumulativeEthGeneratedByDJT[index - 1]);
        } else {
          etherIncomeFromContract.push(value);
        }
      });
