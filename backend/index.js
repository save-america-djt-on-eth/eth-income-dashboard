...
async function updateCache() {
  ...
  try {
    ...
    const generateData = async (timeFrame) => {
      ...
      ethGeneratedByDJT = cumulativeEthGenerated.map((value, index) => {
        if (index > 0) {
          return cumulativeEthGenerated[index] - cumulativeEthGenerated[index - 1];
        } else {
          return value;
        }
      });
      ...
      const ethAddedNonDJT = ethAddedDuringTimeFrame.map((value, index) => value - ethGeneratedByDJT[index]);

      // Make ethGeneratedByDJT cumulative
      for (let i = 1; i < ethGeneratedByDJT.length; i++) {
      ...
      return {
        ...
        ethGeneratedByDJT: ethGeneratedByDJT.slice(1), // Exclude the first value
