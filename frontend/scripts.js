...
function fetchData(timeFrame) {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            document.getElementById("eth-generated-djt").innerText = Number(data.etherIncomeFromContract.slice(-1)[0]).toFixed(4);
            const percentage = ((Number(data.etherIncomeFromContract.slice(-1)[0]) / Number(data.trumpTotalEther)) * 100).toFixed(0);
