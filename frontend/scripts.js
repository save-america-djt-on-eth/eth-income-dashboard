...
app.get('/api/data', async (req, res) => {
    ...
    try {
        ...
        const dailyContractBalances = groupTransactionsByDate(internalTransactions);

        // Ensure we have daily contract balances for the past 7 days
        const contractBalanceData = [];
        const labels = generateTimeLabels(timeFrame);
        labels.forEach(date => {
            contractBalanceData.push(dailyContractBalances[date] || 0);
        });
        ...
        const response = {
            ...
            other: otherData,
            supplyChange,
            contractBalance: dailyContractBalances,
